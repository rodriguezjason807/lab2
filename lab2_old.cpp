...
public:
	...
	Box() {
		...
		w = 20.0f;
		l = 60.0f; // <------ Left off here ( establishing length for cubes )
		dir = 25.0f;
		pos[0] = g.xres / 6.0f;
		pos[1] = g.yres / 0.5f;
		vel[0] = vel[1] = 0.0;
...
//		boxes[m].l = 60.0;
		...
		cout << "box number " << m << ": position (" << boxes[m].pos[0] << ", " << boxes[m].pos[1] << ") and velocity : " << boxes[m].vel[0] << endl;
		cout << "repeating " << endl;
		x = x + 0.50f;
		y = y - 0.5f;
		m++;	
...
//	glPopMatrix();
	...
	for ( int i = 0; i < MAX_BOXES; i++ ) {	
		...
		glBegin(GL_QUADS);
			glVertex2f(-boxes[i].w, -boxes[i].w);
			glVertex2f(-boxes[i].w,  boxes[i].w);
			glVertex2f( boxes[i].w,  boxes[i].w);
			glVertex2f( boxes[i].w, -boxes[i].w);
		glEnd();
	...
	for (int i=0; i<360; i++ ) {
		...
		glVertex2f(circle.pos[0] + circle.radius * cos(circle.theta), circle.pos[1] + circle.radius * sin(circle.theta));	
	}
	glEnd();
	glPopMatrix();
	//
